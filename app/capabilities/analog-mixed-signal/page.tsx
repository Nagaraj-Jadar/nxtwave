...
export const metadata: Metadata = {
  ...
  description: 'Engineering precision across analog, RF, and mixed-signal silicon.',
}
...
const circuitDesign = [
  ...
  'PLL / DLL & Clocking Circuits',
  'References & Bias Circuits',
...
const amsVerification = [
  'Full-Chip AMS Verification',
  'Behavioral Model Development',
  'Verilog-AMS / RNM Modeling',
  'Mixed-Signal Simulation',
  'Analog-Digital Co-Simulation',
  'PVT & Corner Analysis',
  'Monte Carlo & Statistical Analysis',
  'AMS Verification Flow Development',
]

const capabilities = [...circuitDesign, ...layoutDesign, ...amsVerification]

const capabilityIcons = [
  Radio, Zap, Gauge, Waves, CircleDot, Radio, Activity, CircuitBoard,
  Layers3, Boxes, CircuitBoard, Radio, ScanLine, SlidersHorizontal, Settings2, CheckCircle2,
  CircuitBoard, Boxes, GitBranch, Waves, GitBranch, Gauge, BarChart3, Settings2,
]

const deliverables = [
  'Circuit Schematics & Design Data',
  'Custom & Physical Layout Database',
  'AMS / Behavioral Models',
  'Verilog-AMS / RNM Models',
  'Simulation & Verification Reports',
  'Physical Verification & Signoff Reports',
  'Performance & Characterization Reports',
]

const focusAreas = [
  { label: 'Analog', detail: 'Precision', icon: Waves },
  { label: 'RF', detail: 'High frequency', icon: Radio },
  { label: 'AMS', detail: 'Co-simulation', icon: CircuitBoard },
]
          ...
          <div className="relative mx-auto max-w-[1320px] px-4 py-10 sm:px-6 lg:min-h-[calc(100svh-88px)] lg:px-8 lg:py-6">
            ...
            <div className="grid items-center gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:gap-5">
              <div className="relative z-10 pt-2">
                <h1 className="max-w-[650px] text-4xl font-semibold leading-[0.94] tracking-[-0.07em] text-white sm:text-5xl lg:text-[4.2rem]">Analog &amp; Mixed-Signal</h1>
                <p className="mt-4 max-w-[610px] text-xl font-medium leading-relaxed text-sky-100 sm:text-2xl lg:text-[1.65rem]">Engineering Precision Across Analog, RF &amp; Mixed-Signal</p>
                <div className="mt-6 max-w-[570px] space-y-3 text-[15px] leading-[1.8] text-slate-300" style={{ textAlign: 'justify', textJustify: 'inter-word' }}>
                  <p style={{ textAlign: 'justify', textJustify: 'inter-word' }}>We deliver end-to-end Analog &amp; Mixed-Signal engineering across Analog Circuit Design, Layout Design, and AMS Verification. Our expertise spans high-performance analog, RF, power, clocking, data converters, and mixed-signal technologies - helping customers move from concept to reliable silicon.</p>
                </div>
                <div className="mt-6 flex flex-wrap items-center gap-3"><Link href="/contact" className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-[0_18px_35px_rgba(15,23,42,0.22)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-sky-700">Talk to our team<ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><span className="rounded-full border border-sky-300/30 bg-white/10 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.24em] text-sky-200">Concept -&gt; Silicon</span></div>
              </div>

              <div className="relative flex min-h-[350px] items-center justify-center overflow-visible lg:min-h-[465px]" aria-label="Analog and mixed-signal engineering visualization">
                <div className="relative h-[350px] w-[130%] max-w-[620px] scale-[0.74] lg:h-[465px] lg:w-full lg:scale-100">
                  ...
                  <svg className="absolute inset-0 h-full w-full" viewBox="0 0 620 520" fill="none" aria-hidden="true">
                    <defs><linearGradient id="amsTrace" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#38bdf8" stopOpacity="0" /><stop offset="0.5" stopColor="#67e8f9" /><stop offset="1" stopColor="#2563eb" stopOpacity="0" /></linearGradient></defs>
                    <path d="M28 390H142L218 330M592 390H478L402 330M68 452H180L238 370M552 452H440L382 370" stroke="url(#amsTrace)" strokeWidth="2" className="animate-trace-flow" />
                    <path d="M170 128L250 220M450 128L370 220M120 270L228 300M500 270L392 300" stroke="#67e8f9" strokeOpacity="0.28" />
                    <path d="M94 190C120 155 140 225 166 190S212 155 238 190M382 190C408 155 428 225 454 190S500 155 526 190" stroke="#67e8f9" strokeWidth="2" strokeDasharray="6 9" className="animate-trace-flow" />
                  ...
                  <div className="absolute left-1/2 top-[8%] w-[230px] -translate-x-1/2 rounded-xl border border-cyan-300/35 bg-[#102b55]/75 px-4 py-3 text-center shadow-[0_18px_35px_rgba(0,0,0,0.22)] backdrop-blur-sm"><div className="flex items-center justify-center gap-2 text-[9px] font-semibold uppercase tracking-[0.22em] text-cyan-200"><Waves className="h-3.5 w-3.5" /> Analog signal</div><svg className="mt-3 h-8 w-full" viewBox="0 0 210 32" preserveAspectRatio="none"><path d="M0 16C12 2 24 30 36 16S60 2 72 16S96 30 108 16S132 2 144 16S168 30 180 16S198 4 210 16" stroke="#67e8f9" strokeWidth="2" className="animate-trace-flow" /></svg></div>
                  <div className="absolute left-1/2 top-[31%] h-[178px] w-[276px] -translate-x-1/2 rounded-2xl border border-cyan-200/70 bg-[linear-gradient(135deg,#314866_0%,#0a1428_58%,#1c3555_100%)] p-4 shadow-[inset_0_0_25px_rgba(96,165,250,0.24),0_0_30px_rgba(56,189,248,0.2)]"><div className="h-full rounded-xl border border-sky-300/30 bg-[#071426] p-3"><div className="flex items-center justify-between text-[9px] font-mono uppercase tracking-[0.2em] text-sky-200"><span>AMS CORE</span><CheckCircle2 className="h-4 w-4 text-emerald-300" /></div><div className="mt-4 grid grid-cols-3 gap-2 text-center text-[8px] uppercase tracking-[0.12em] text-slate-300">{['PLL / DLL', 'ADC / DAC', 'RF I/O', 'LDO', 'BIAS', 'MIXED'].map((item) => <span key={item} className="border border-sky-300/25 bg-sky-200/10 px-1 py-2">{item}</span>)}</div></div></div>
                  <div className="absolute bottom-[12%] left-1/2 w-[300px] -translate-x-1/2 rounded-xl border border-blue-300/35 bg-[#0d2348]/85 px-4 py-3 shadow-[0_18px_35px_rgba(0,0,0,0.24)] backdrop-blur-sm"><div className="flex items-center justify-between text-[9px] font-semibold uppercase tracking-[0.22em] text-blue-100"><span>Silicon confidence</span><span className="text-emerald-300">Verified</span></div><svg className="mt-3 h-9 w-full" viewBox="0 0 300 36" preserveAspectRatio="none"><path d="M0 25H18V14H42V25H68V19H92V25H116V8H142V25H166V16H194V25H220V11H248V25H274V16H300" stroke="#67e8f9" strokeWidth="2" className="animate-trace-flow" /></svg><div className="mt-1 flex justify-between text-[8px] uppercase tracking-[0.15em] text-slate-400"><span>PVT</span><span>Monte Carlo</span><span>Sign-off</span></div></div>
                  <div className="absolute bottom-[3%] left-1/2 -translate-x-1/2 text-center text-[9px] font-semibold uppercase tracking-[0.28em] text-sky-300/80">Precision <span className="px-2 text-cyan-300">-&gt;</span> Confidence</div>
                </div>
              </div>
            </div>
          </div>
