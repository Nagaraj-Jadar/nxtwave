...
import { ArrowRight, BarChart3, CheckCircle2, Code2, Layers3, ShieldCheck } from 'lucide-react'
...
const capabilities = [
  ...
  'Regression infrastructure development',
  'Automated regression management',
  'Debug and root-cause analysis',
  'Verification sign-off support',
]
  ...
  return (
    <>
      ...
      <main className="bg-[#f4f8ff] text-slate-900">
        ...
        <section className="bg-[#f7faff] py-16 sm:py-20 lg:py-24">
          <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8">
            <div className="mb-10 border-b border-sky-200 pb-8">
              <h2 className="max-w-[820px] text-[3rem] font-semibold leading-[0.94] tracking-[-0.06em] text-slate-950 sm:text-[4rem] lg:text-[4.5rem]">Our <span className="text-sky-600">Capabilities</span></h2>
              <p className="mt-5 max-w-[760px] font-serif text-[1.35rem] font-semibold leading-tight text-slate-900 sm:text-[1.65rem]">Verification engineered for confidence before silicon.</p>
            </div>
            <div className="grid gap-2.5 md:grid-cols-2 md:gap-x-4 md:gap-y-3">
              {capabilities.map((item) => (
                <div key={item} className="flex min-h-[58px] items-center gap-4 rounded-lg border border-sky-100 bg-white px-3 py-2.5 shadow-[0_10px_24px_-22px_rgba(15,43,93,0.55)] sm:min-h-[62px] sm:px-4">
                  <span className="flex h-9 w-9 flex-none items-center justify-center rounded-lg bg-[#edf4ff] text-sky-600"><CheckCircle2 className="h-[18px] w-[18px]" strokeWidth={1.6} aria-hidden="true" /></span>
        ...
        <section className="relative overflow-hidden bg-[linear-gradient(135deg,#07142d_0%,#0c2348_54%,#071326_100%)] py-16 text-white sm:py-20 lg:py-24">
          ...
          <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8">
            <div className="relative grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
              <div>
                <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-cyan-300">Verification methodology</div>
                <h2 className="mt-4 max-w-[580px] text-3xl font-semibold tracking-[-0.05em] text-white sm:text-4xl">Confidence built into every regression.</h2>
                <div className="mt-8 h-px w-24 bg-cyan-300/70" />
                <p className="mt-6 max-w-[420px] text-[15px] leading-7 text-slate-300">From planning through closure, every verification decision is anchored in measurable evidence and reusable engineering practice.</p>
              </div>
              <div className="space-y-4">
                {principles.map(({ heading, text, icon: Icon }) => (
                  <div key={heading} className="flex gap-4 border-b border-sky-300/20 pb-5">
                    <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-cyan-300/30 bg-cyan-300/10 text-cyan-200"><Icon className="h-5 w-5" aria-hidden="true" /></div>
                    <div><h3 className="font-sans text-sm font-semibold text-white">{heading}</h3><p className="mt-2 text-[13px] leading-6 text-slate-300">{text}</p></div>
                  </div>
                ))}
              </div>
              <div className="border-t border-sky-300/20 pt-8 lg:col-span-2">
                <div className="flex items-center justify-between"><div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-cyan-300">Deliverables</div><div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-sky-300">Ready to ship</div></div>
                <div className="mt-6 grid gap-x-10 md:grid-cols-2">{deliverables.map((item) => <div key={item} className="flex items-start gap-3 border-b border-sky-300/15 py-4"><span className="mt-1 text-cyan-300">↳</span><p className="text-[15px] leading-7 text-slate-200">{item}</p></div>)}</div>
              </div>
            </div>
          </div>
        </section>
